document.addEventListener('DOMContentLoaded', () => {
    ...
    const shoeLaces = document.querySelectorAll('#laces path');
    ...
    const soleSwatches = document.querySelectorAll('#soleSwatches .swatch');
    const laceSwatches = document.querySelectorAll('#laceSwatches .swatch');
    ...
    function updateShoeColor(part, color) {
        if (part instanceof NodeList || Array.isArray(part)) {
            part.forEach(p => {
                p.style.stroke = color; // For laces
                animatePart(p);
            });
        } else {
    ...
    handleSwatchClick(soleSwatches, shoeSole, 'soleColor');
    handleSwatchClick(laceSwatches, shoeLaces, 'laceColor');
