...
import { WG_ENV, WG_INITIAL_ENV } from '../utils/config';
...
export async function connect() {
  DB_DEBUG('Connecting to database...');
  await migrate();
  DB_DEBUG('Database migration completed.');

  const dbService = new DBService(db);
  DB_DEBUG('Database service initialized.');

  if (WG_INITIAL_ENV.ENABLED) {
    DB_DEBUG('Running initial setup...');
    await initialSetup(dbService);
    DB_DEBUG('Initial setup completed.');
  }

  if (WG_ENV.DISABLE_IPV6) {
    DB_DEBUG('Warning: Disabling IPv6...');
    await disableIpv6(db);
    DB_DEBUG('IPv6 disabled in hooks.');
  }

  DB_DEBUG('Database connection established successfully.');
  return dbService;
}
