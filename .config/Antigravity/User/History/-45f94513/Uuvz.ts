...
export function nextIP(
  ...
  console.debug(`Finding next available IP for IPv${version}, CIDR: ${cidr.start} - ${cidr.end}, total clients: ${clients.length}`);
  let address;
  for (let i = cidr.start + 2n; i <= cidr.end - 1n; i++) {
    const currentIp = stringifyIp({ number: i, version: version });
    console.debug(`Checking IP: ${currentIp}`);
    ...
    if (!client) {
      address = currentIp;
      console.debug(`Found available IP: ${address}`);
      break;
    }
  }

  if (!address) {
    console.error(`Maximum number of clients reached for IPv${version}`, {
 ...
 * @returns true if the host is a valid IPv6 address, false otherwise
 */
export function isIPv6Address(host: string): boolean {
  // Remove square brackets if present
  const cleanHost = host.replace(/^\[|\]$/g, '');

  // Check if it contains at least one colon (IPv6 addresses have colons)
...
/**
 * Format a host for use in a URL, wrapping IPv6 addresses in square brackets
 * @param host The host string to format
 * @returns The formatted host string
 */
export function formatHostForUrl(host: string): string {
  if (isIPv6Address(host)) {
    // Remove existing brackets if present, then add them back
    const cleanHost = host.replace(/^\[|\]$/g, '');
    return `[${cleanHost}]`;
  }
  return host;
