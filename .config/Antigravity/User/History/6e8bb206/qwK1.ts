...
class WireGuard {
  ...
  async Startup() {
    ...
    try {
      ...
      WG_DEBUG('Fetching interface from database...');
      let wgInterface = await Database.interfaces.get();
      WG_DEBUG('Interface fetched successfully:', wgInterface.name);
      ...
      if (
        ...
        WG_DEBUG('Private and public keys generated successfully.');

        await Database.interfaces.updateKeyPair(privateKey, publicKey);
        wgInterface = await Database.interfaces.get();
        WG_DEBUG('New Wireguard Keys generated and saved successfully.');
      } else {
        WG_DEBUG('Using existing WireGuard keys.');
      }
      ...
      if (WG_ENV.WG_EXECUTABLE === 'awg' && wgInterface.h1 === 0) {
        ...
        WG_DEBUG('AmneziaWG obfuscation parameters generated and saved.');
      }

      WG_DEBUG(`Starting Wireguard Interface ${wgInterface.name}...`);
      await this.#saveWireguardConfig(wgInterface);
      WG_DEBUG('WireGuard config saved.');

      await wg.down(wgInterface.name).catch((err) => {
        WG_DEBUG('Interface was not running, proceeding to start:', err?.message);
      });
      WG_DEBUG('Attempting to bring up WireGuard interface...');
      await wg.up(wgInterface.name).catch((err) => {
        WG_DEBUG('Error during WireGuard up operation:', err);
        if (
          err &&
          err.message &&
      ...
      WG_DEBUG('WireGuard interface started successfully.');
      ...
      WG_DEBUG('Error during WireGuard startup:', error);
      throw error;
    }
  }
